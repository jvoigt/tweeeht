...
export class StaticMongoCollectionService implements Messager {
  ...
  constructor(
    ...
    @InjectModel('StaticCollection')
    private staticCollectionModel: Model<StaticCollection>,
  ) {
  ...
  create(
    createStaticCollectionDto: CreateStaticCollectionDto,
    owners?: User[],
  ): Observable<StaticCollection> {
    ...
    const createdStaticCollection = new this.staticCollectionModel(
      newStaticCollection,
    );

    return from(createdStaticCollection.save()).pipe(
  ...
  findOnebyId(id: string): Observable<StaticCollection> {
    return from(this.staticCollectionModel.findById(id).exec()).pipe(
      catchError((error) => {
  ...
  findOnebyName(name: string): Observable<StaticCollection> {
    return from(this.staticCollectionModel.findOne({ name }).exec());
  }
    ...
    return this.findOnebyId(id).pipe(
      switchMap((oldStaticCollection: StaticCollection) => {
        ...
        const updatedStaticCollection = this.staticCollectionModel.updateOne(
          { _id: oldStaticCollection._id },
          createStaticCollectionDto,
        );
